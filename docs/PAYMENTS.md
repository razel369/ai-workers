...
אסמכתאות Bit ו-PayPal נרשמות גם ב-ledger גלובלי של הפלטפורמה עם מפתח ייחודי
`(provider, reference)`. אותה אסמכתה יכולה להתאים רק לאותו tenant ולעובד המקורי;
ניסיון לשייך אותה לעובד אחר או לעסק אחר נכשל לפני שינוי ה-entitlement.

## עקיפת אדמין

האדמין יכול לאשר ידנית ב-`#/admin` גם כש-webhook נכשל, אבל אישור Bit/PayPal
עדיין עובר דרך ה-ledger הגלובלי ואינו יכול לעקוף בעלות קודמת על אסמכתה.
