...
בפרודקשן, נתונים נשמרים ב-SQLite ובתיקיות דיירים על אחסון קבוע בנתיב `/app/data`. יעד הפריסה הנוכחי הוא Oracle Always Free; אין להזין נתוני לקוחות לפני שהאחסון, הגיבוי מחוץ לשרת והשחזור אומתו. גישת אדמין מוגנת ב-`ADMIN_TOKEN`. מומלץ לסובב מפתחות API לאחר חשיפה.
